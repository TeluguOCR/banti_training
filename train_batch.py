...
aux_file_name = None
prms_file_names = []
write_to_file = sys.argv[-1] == '1'
if len(sys.argv) > 3:
    if sys.argv[3].endswith('.bz2'):
        aux_file_name = sys.argv[3]
        prms_file_names = sys.argv[4:]
    else:
        prms_file_names = sys.argv[3:]
if len(prms_file_names) > 0 and prms_file_names[-1] == '1':
    prms_file_names.pop()
...
batch_sz = 20  # tr_prms['BATCH_SZ']
corpus_sz, _, imgsz, _ = data_x.shape
n_train = int(corpus_sz * .75)  # tr_prms['TRAIN_ON_FRACTION']
...
def train(prms_file_name, out_to_file=True):
    ...
    def do_test():
        ...
        test_err, aux_test_err = test_wrapper(test_fn_te(i) for i in next(test_indices))
        trin_err, aux_trin_err = test_wrapper(test_fn_tr(i) for i in next(trin_indices))
        print("{:5.2f}%  ({:5.2f}%)      {:5.2f}%  ({:5.2f}%)".format(trin_err, aux_trin_err, test_err, aux_test_err))
        if out_to_file:
            print("{:5.2f}%".format(test_err), end='.. ', file=stdout_orig)
