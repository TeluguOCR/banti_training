...
def split_file_name(file_path):
    ...
    try:
        ...
        dtbpairs = [(dtbs[j], dtbs[j + 1]) for j in range(0, len(dtbs), 2)]
        ...
        for font_style, glyph_list in list(self.glyphs.items()):
            for glyph in glyph_list:
                for dtop, dbot in glyph.dtopbot_pairs:
                    yield self.scaler(BasicGlyph((glyph.img, dtop, dbot))), font_style
    ...
    def get_one_per_style(self, ):
        for font_style, glyph_list in list(self.glyphs.items()):
            try:
                ...
                print(f"One of the files in {font_style} is bad. Fix it.")
                traceback.print_exc()
                raise

    def get_one_per_style_relative(self, ):
        for font_style, glyph_list in list(self.glyphs.items()):
            glyph = choice(glyph_list)
            dtop, dbot = choice(glyph.dtopbot_pairs)
            scaled_glyph = self.scaler(BasicGlyph((glyph.img, dtop, dbot)))
            assert scaled_glyph is not None

            # Find scaled dtop, dbot for all imgs
            scaled_dtopbots = []
            for glyph in glyph_list:
                for dtop, dbot in glyph.dtopbot_pairs:
                    scaled_dtopbots.append(self.scaler.get_scaled_dtop_dbot(BasicGlyph((glyph.img, dtop, dbot))))
            ...
            yield (scaled_glyph,
                   font_style,
                   np.round(np.array((highs, lowws)), 3).tolist())
...
def process_root_dir(root_dir, ndirs=np.inf):
...
if len(sys.argv) < 6:
    print('''Usage:
{0} <images.tar.gz/dir> <scaler.scl> <codes.lbl> <image_sampler> <prefix> [max_dirs=np.inf]
...
######################################## Process Arguments
_, tar_file, scaler_prms_file, codes_file, image_sampler, prefix = sys.argv[:6]
dirs_dir = '/tmp/rakesha/'
max_dirs = np.inf if len(sys.argv) == 6 else int(sys.argv[6])
...
labels, imgs, meta, aux = process_root_dir(dirs_dir, max_dirs)

################################### Apply a random permutation
n_samples = len(labels)
print("Found a total of", n_samples, "samples")

print("Applying Random Permutation...")
...
for ii, iimg in enumerate(imgs):
    imgs[ii] = iimg.astype("int").tolist()

################################### Save
print("Saving to zip files...")
...
with open(prefix+'.images.json', 'w') as images_fp:
    print("Saving", prefix+'.images.json')
    json.dump(imgs, images_fp)
